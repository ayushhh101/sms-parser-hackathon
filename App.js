...
import {
  ...
  RefreshControl, StatusBar, Alert, Clipboard, Platform
} from 'react-native';
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const subscription = startSmsListener((newMsg) => {
      ...
      // Alert for OTP
      if (newMsg.isOTP) {
        Alert.alert('🔐 New OTP', `Code: ${newMsg.otp}\nFrom: ${newMsg.sender}`, [
          { text: 'Copy', onPress: () => copyOTP(newMsg.otp) },
          { text: 'OK' },
        ]);
      }
    });
  ...
  const loadSMS = async () => {
    try {
      const msgs = await fetchSMS({ maxCount: 150 });
      setTransactions(msgs);
      setStats(getStats(msgs));
    } catch (e) {
      ...
      <SafeAreaProvider>
        <SafeAreaView style={s.container}>
          ...
          <View style={s.center}>
            ...
            <Text style={s.title}>SMS Permission Needed</Text>
            <Text style={s.subtitle}>Allow SMS access to parse your transactions</Text>
...
const s = StyleSheet.create({
  ...
  header: { padding: 16, borderBottomWidth: 1, borderColor: C.border },
